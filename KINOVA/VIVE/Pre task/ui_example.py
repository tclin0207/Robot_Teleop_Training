...
class videoThread(threading.Thread):
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # frame_eye_in_hand = cv2.resize(frame_rs1, (800, 450), interpolation = cv2.INTER_AREA)

            # sequence hint in main view
            mid_x_ar, mid_y_ar, corners = self.detect_markers(frame_rs2, 7)
            if mid_x_ar != 0:
                cv2.putText(frame_rs2, '1', (int(mid_x_ar)-10, int(mid_y_ar)-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)

            mid_x_ar, mid_y_ar, corners = self.detect_markers(frame_rs2, 8)
            if mid_x_ar != 0:
                cv2.putText(frame_rs2, '2', (int(mid_x_ar)-10, int(mid_y_ar)-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)

            mid_x_ar, mid_y_ar, corners = self.detect_markers(frame_rs2, 9)
            if mid_x_ar != 0:
                cv2.putText(frame_rs2, '3', (int(mid_x_ar)-10, int(mid_y_ar)-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)

            mid_x_ar, mid_y_ar, corners = self.detect_markers(frame_rs2, 10)
            if mid_x_ar != 0:
                cv2.putText(frame_rs2, '4', (int(mid_x_ar)-10, int(mid_y_ar)-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)

            mid_x_ar, mid_y_ar, corners = self.detect_markers(frame_rs2, 11)
            if mid_x_ar != 0:
                cv2.putText(frame_rs2, '5', (int(mid_x_ar)-10, int(mid_y_ar)-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)

            # create center in eye-in-hand view
            # cv2.putText(frame_eye_in_hand, '+', (300, 300), cv2.FONT_HERSHEY_SIMPLEX, 7, (51, 255, 255), 5, cv2.LINE_AA)

            # Different detection level for each marker
            # mid_x_ar, mid_y_ar, corners = self.detect_markers(frame_eye_in_hand, 7)
            # level_1 = self.dist_check(corners, 50)
            # if mid_x_ar != 0:
            #     if level_1:
            #         cv2.circle(frame_eye_in_hand, (int(mid_x_ar), int(mid_y_ar)), 70, (255,255,255), 5)
            ...
            mid_x_ar, mid_y_ar, corners = self.detect_markers(resized_frame, 7)
            # print(mid_x_ar, mid_y_ar)

            print_ = self.dist_check(corners,50)
            if mid_x_ar != 0:
                if print_:
                    cv2.circle(resized_frame, (int(mid_x_ar), int(mid_y_ar)), 70, (255,255,255), 5)

            # create center in eye-in-hand view
            cv2.putText(resized_frame, '+', (120, 120), cv2.FONT_HERSHEY_SIMPLEX, 3, (51, 255, 255), 2, cv2.LINE_AA)  
            ...
            cv2.resizeWindow('Main View', 960, 540)
            cv2.imshow('Main View', frame_rs2)

            # cv2.namedWindow('Eye-in-hand View', cv2.WINDOW_GUI_NORMAL)
            # cv2.resizeWindow('Eye-in-hand View', 960, 540)
            # cv2.imshow('Eye-in-hand View', frame_eye_in_hand)
